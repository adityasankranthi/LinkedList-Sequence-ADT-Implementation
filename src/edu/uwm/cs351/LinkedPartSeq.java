...
public class LinkedPartSeq implements Robot, Cloneable {
    ...
    public void start(String function) {
        // Assert the invariant before
        assert wellFormed() : "invariant broken in start";
        this.function = function;
        precursor = null;
        if (head != null && function != null) {
            Node cur = head;
            while (cur != null && !function.equals(cur.function)) {
                precursor = cur;
                cur = cur.next;
            }
        }

        // Assert the invariant after
    ...
    public void advance() {
        ...
        precursor = getCursor();
        if (this.function != null && getCursor() != null && !this.function.equals(getCursor().function)) {
            Node lag = null;
    ...
    public boolean addPart(String function, Part part) {
        ...
        start(function);
        while (isCurrent()) {
            precursor = tail; // Move to the last part
        }
        addAfter(part);
